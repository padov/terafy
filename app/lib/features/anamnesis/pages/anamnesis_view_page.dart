...
class AnamnesisViewPage extends StatelessWidget {
  ...
  final AnamnesisBloc? bloc;

  const AnamnesisViewPage({super.key, required this.patientId, this.anamnesisId, this.bloc});
  ...
  Widget build(BuildContext context) {
                  ...
                  ElevatedButton(
                    onPressed: () {
                      context.read<AnamnesisBloc>().add(
                        anamnesisId != null ? LoadAnamnesisById(anamnesisId!) : LoadAnamnesisByPatientId(patientId),
                      );
                    },
    ...
    return BlocProvider(
      ...
      )..add(anamnesisId != null ? LoadAnamnesisById(anamnesisId!) : LoadAnamnesisByPatientId(patientId)),
      child: content,
    );
  }

  Widget _buildAnamnesisView(BuildContext context, Anamnesis anamnesis, AnamnesisTemplate? template) {
                ...
                children: [
                  ...
                  if (template?.description != null)
                    Padding(
                      ...
                      child: Text(template!.description!, style: TextStyle(fontSize: 12, color: Colors.grey[600])),
                    ),
                ],
              ),
            ),
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Informações', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
  ...
  Widget _buildSectionCard(AnamnesisSection section, Map<String, dynamic> data) {
    final sortedFields = List.from(section.fields)..sort((a, b) => a.order.compareTo(b.order));
          ...
          children: [
            ...
            if (section.description != null)
              Padding(
                ...
                child: Text(section.description!, style: TextStyle(fontSize: 12, color: Colors.grey[600])),
              ),
            const Divider(),
            const SizedBox(height: 8),
            ...sortedFields
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(_formatValue(value, field.type), style: const TextStyle(fontSize: 14)),
        ],
      ),
    );
  }
              ...
              return Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      flex: 2,
                      child: Text(entry.key, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                    ),
                    Expanded(
                      flex: 3,
                      child: Text(entry.value?.toString() ?? 'Não informado', style: const TextStyle(fontSize: 13)),
                    ),
                  ],
                ),
              );

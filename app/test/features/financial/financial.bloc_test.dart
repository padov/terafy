...
void main() {
  group('FinancialBloc', () {
    ...
    blocTest<FinancialBloc, FinancialState>(
      'emite FinancialData quando LoadFinancialSummary é adicionado',
      build: () {
      ...
      act: (bloc) => bloc.add(
        LoadFinancialSummary(startDate: DateTime.now().subtract(Duration(days: 30)), endDate: DateTime.now()),
      ),
      expect: () => [const FinancialLoading(), isA<FinancialData>()],
    );
    ...
    blocTest<FinancialBloc, FinancialState>(
      ...
      expect: () => [const FinancialLoading(), isA<FinancialError>()],
    );
  });
}

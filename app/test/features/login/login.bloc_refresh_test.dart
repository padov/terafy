...
void main() {
  ...
  final client = Client(id: '1', name: 'Therapist Test', email: 'therapist@example.com', accountId: 123);
  ...
  LoginBloc buildBloc() {
    ...
    return LoginBloc(sessionManager: sessionManager);
  }
  ...
  group('LoginBloc refresh flow', () {
    ...
    test('attempts refresh when token invalid and emits LoginSuccess after renewal', () async {
      final bloc = buildBloc();
      addTearDown(bloc.close);
      ...
      final responses = <AuthResult>[const AuthResult(error: 'invalid token'), AuthResult(client: client)];

      when(() => getCurrentUserUseCase()).thenAnswer((_) async => responses.removeAt(0));

      when(
        () => refreshTokenUseCase.call('refresh-token'),
      ).thenAnswer((_) async => const AuthResult(authToken: 'new-access-token', refreshAuthToken: 'new-refresh-token'));
    ...
    test('cleans up tokens when refresh fails', () async {
      ...
      when(() => getCurrentUserUseCase()).thenAnswer((_) async => const AuthResult(error: 'invalid token'));
      ...
      expect(collectedStates.length, 1);
      expect(collectedStates.first, isA<LoginFailure>());

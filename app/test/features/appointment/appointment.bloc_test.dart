...
void main() {
  group('AppointmentBloc', () {
    ...
    test('estado inicial é AppointmentInitial', () {
      expect(bloc.state, const AppointmentInitial());
    });

    blocTest<AppointmentBloc, AppointmentState>(
      'emite AppointmentLoaded quando LoadAppointment é adicionado',
      build: () {
        when(
      ...
      act: (bloc) =>
          bloc.add(LoadAppointments(startDate: DateTime.now(), endDate: DateTime.now().add(Duration(days: 7)))),
      expect: () => [const AppointmentLoading(), isA<AppointmentLoaded>()],
    );

    blocTest<AppointmentBloc, AppointmentState>(
      'emite AppointmentError quando LoadAppointment falha',
      build: () {
        when(
          () => getAppointmentsUseCase(
            start: any(named: 'start'),
      ...
      act: (bloc) =>
          bloc.add(LoadAppointments(startDate: DateTime.now(), endDate: DateTime.now().add(Duration(days: 7)))),
      expect: () => [const AppointmentLoading(), isA<AppointmentError>()],
    );
  });
}
